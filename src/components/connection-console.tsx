...
const AUTH_METHODS: {
  ...
  {
    key: "ropc",
    title: "ROPC (Service Account)",
    note: "Resource Owner Password Credentials. Best for backend integration using service account credentials. SSO-exempt.",
    recommended: true,
  },
  {
    key: "pkce",
...
const PREREQUISITES = [
  {
    title: "Service Account is SSO-exempt",
    detail: "Verify the Svc-itsmsdg@ingrammicro.com user can login with password credentials directly.",
  },
  {
    title: "Application Registry entry created",
    detail: "System OAuth → Application Registry → “Create an OAuth API endpoint for external clients”",
  },
  {
    title: "Client ID and Secret Generated",
    detail: "Take the Client ID and Secret from the registry record and save them securely.",
  },
  {
    title: "OAuth Scopes assigned",
    detail: "Ensure useraccount, rest_api and table_api scopes are assigned to the registry.",
  },
  {
    title: "REST / Table API ACLs",
    detail: "Fulfiller-level read ACLs across incident, problem, change_request, and kb_knowledge.",
  },
];
...
const PHASE2_FEATURES = [
  ...
  "SSO-exempt service account login using ServiceNow ROPC credentials.",
...
const preflightSteps = (base: string): PreflightStep[] => [
  ...
  { id: "endpoints", label: "Derive OAuth endpoints", detail: `${base}/oauth_token.do`, status: "pending" },
  { id: "client", label: "Check client registration", detail: "Client ID and Secret are configured", status: "pending" },
  { id: "ropc", label: "Establish OAuth handshake", detail: "Exchanging credentials for bearer tokens", status: "pending" },
  { id: "scope", label: "Verify active token", detail: "Checking live token validity against endpoint", status: "pending" },
];
...
export function ConnectionConsole({
  ...
  const [clientId, setClientId] = useState(connection.clientIdSet ? "••••••••••••••••••••••••••••••••" : "");
  const [clientSecret, setClientSecret] = useState(connection.hasSecret ? "••••••••••••••••••••••••••••••••" : "");
  const [showSecret, setShowSecret] = useState(false);
  const [authMethod, setAuthMethod] = useState<AuthMethod>(connection.authMethod ?? "ropc");
  const [serviceUsername, setServiceUsername] = useState(connection.serviceUsername ?? "");
  const [servicePassword, setServicePassword] = useState(connection.hasPassword ? "••••••••••••••••" : "");
  ...
  async function save() {
    ...
    try {
      const response = await fetch("/api/connection", {
        ...
        body: JSON.stringify({
          instanceUrl,
          clientId: clientId === "••••••••••••••••••••••••••••••••" ? (connection.clientIdSet ? undefined : "") : clientId,
          clientSecret: clientSecret === "••••••••••••••••••••••••••••••••" ? undefined : clientSecret,
          keepSecret: clientSecret === "••••••••••••••••••••••••••••••••" || !clientSecret,
          serviceUsername,
          servicePassword: servicePassword === "••••••••••••••••" ? undefined : servicePassword,
          keepPassword: servicePassword === "••••••••••••••••" || !servicePassword,
          redirectUri,
          authMethod,
          scope: scopeString,
        }),
      ...
      if (!response.ok || !payload.ok) {
        setSaveState({ kind: "error", errors: payload.errors ?? ["Unable to save the connection details."] });
        return;
      }
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="reveal flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between" style={{ "--d": "0s" } as React.CSSProperties}>
        <div>
          <p className="mt-2 max-w-2xl text-[15px] leading-7 text-ink/70">
            Capture everything the live REST integration needs — instance URL, OAuth client details,
            Service Account credentials. This enables live record fetching from ServiceNow directly into the Workbench.
          </p>
        </div>
        ...
        <div className="flex shrink-0 items-center gap-3 rounded-2xl border border-line bg-white px-5 py-4 shadow-sm">
          <div className="text-right">
            ...
            <p className={`mt-1 font-display text-lg font-bold ${connection.configured ? "text-leaf-deep" : "text-amber-600"}`}>
              {connection.configured ? "Connection Saved" : "Not configured yet"}
            </p>
      ...
      <section className="reveal grid gap-6 xl:grid-cols-[minmax(0,1fr)_430px]" style={{ "--d": "0.16s" } as React.CSSProperties}>
        <div className="rounded-3xl border border-line bg-white p-6 shadow-[0_20px_50px_-30px_rgba(17,52,44,0.35)] sm:p-8">
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="relative mt-2">
                <input
                  ...
                  placeholder="https://ingrammicrotest.service-now.com"
            ...
            {authMethod === "ropc" && (
              <div className="rounded-2xl border border-line bg-leaf-soft/25 p-5 space-y-4">
                <p className="font-mono text-[11px] font-semibold uppercase tracking-[0.16em] text-leaf-deep">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Svc-itsmsdg@ingrammicro.com"
                  ...
                  <div>
                    ...
                    <div className="relative mt-2">
                      <input
                        ...
                        placeholder={connection.hasPassword ? "••••••••" : "Service Account Password"}
            ...
            {authMethod !== "basic" && (
              <div className="grid gap-5 sm:grid-cols-2">
                ...
                {authMethod === "client_secret" || authMethod === "ropc" ? (
                  <div>
                    <label className="font-mono text-[11px] font-semibold uppercase tracking-[0.16em] text-ink/55">
                      Client secret
                    </label>
            ...
            <div className="flex flex-wrap items-center gap-4 border-t border-line pt-5">
              ...
              >
                {saveState.kind === "saving" ? "Saving credentials…" : "Save connection details"}
              </button>
      ...
      <section className="reveal" style={{ "--d": "0.32s" } as React.CSSProperties}>
        ...
        <div className="rounded-3xl border border-line bg-mist/30 p-6 sm:p-8">
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            <div className="max-w-xl">
              <span className="inline-flex items-center gap-2 rounded-full border border-leaf/60 bg-white px-3 py-1 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-leaf-deep">
                <span className="size-1.5 rounded-full bg-leaf dot-live" />
                Live and fully supported
              </span>
              <h3 className="mt-4 font-display text-2xl font-extrabold tracking-tight text-pine">
                Pull records and related lists straight from ServiceNow
              </h3>
              <p className="mt-3 text-sm leading-6 text-ink/65">
                Your connection is fully enabled. To fetch any incident, problem, change, or requested item,
                switch to the live ServiceNow fetcher directly in the Analysis Workbench. The app securely manages ROPC token lifecycles with automatic refresh.
              </p>
            ...
            <div className="w-full max-w-lg shrink-0 rounded-2xl border border-line bg-white p-4 shadow-sm">
              ...
              <p className="mt-3 rounded-lg bg-mist/80 px-3 py-2 font-mono text-[10.5px] leading-4 text-ink/55">
                + related incident tasks are automatically correlated as live XML context.
              </p>
            </div>
          </div>
        </div>
