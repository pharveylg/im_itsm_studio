...
export type AuthMethod = "pkce" | "client_secret" | "basic" | "ropc";
...
export type ConnectionSettings = {
  ...
  scope: string;
  // Added for live ServiceNow REST credential input
...
export function validateSettings(input: ConnectionSettings): string[] {
  ...
  if (!["pkce", "client_secret", "basic", "ropc"].includes(input.authMethod)) {
  ...
  if (input.authMethod === "ropc") {
    if (!input.serviceUsername?.trim()) {
      errors.push("Service Username is required for ROPC credentials.");
    }
    if (!input.servicePassword?.trim() && !input.keepPassword) {
      errors.push("Service Password is required for ROPC credentials.");
    }
  }
