      ...
      try {
        ...
        payload = {
          ...
          error: looksLikeTimeout
            ? `The analysis timed out on the server (status ${response.status}). Reduce the packet size, untick "Include bounded raw XML excerpts", enable Turbo mode, select fewer modules, or use a faster model. On Vercel Hobby the function limit is 60s; Pro allows up to 300s.`
            : looksTooLarge
            ...
            <section className="rounded-[2rem] border border-white/80 bg-white/95 p-6 shadow-xl shadow-slate-200/70">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-2 text-sm leading-6 text-slate-600">Add exports manually, or use the live fetcher below if connected.</p>
                </div>
              ...
              {/* Live ServiceNow Fetcher */}
              <LiveServiceNowFetcher
                onFetched={(filename, xml) => {
                  const id = `live-${filename}-${Date.now()}`;
                  setFiles((current) => {
                    const existing = new Set(current.map((file) => file.name));
                    if (existing.has(filename)) {
                      return current.map((file) => file.name === filename ? { ...file, xml, bytes: new TextEncoder().encode(xml).length } : file);
                    }
                    return [...current, { id, name: filename, xml, bytes: new TextEncoder().encode(xml).length }];
                  });
                }}
              />
             ...
             <section className="rounded-[2rem] border border-white/80 bg-white/95 p-6 shadow-xl shadow-slate-200/70">
               ...
               <label className="mt-4 flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-3">
                 <input type="checkbox" checked={turbo} onChange={(event) => setTurbo(event.target.checked)} className="mt-1 size-4 rounded border-amber-300" />
                 <span>
                   <span className="block text-sm font-bold text-amber-900">Turbo mode (Vercel Hobby)</span>
                   <span className="mt-1 block text-xs leading-5 text-amber-700">Truncates guidelines, skips raw context in prompt, reduces output budget to 8,000 tokens. Recommended when the analysis times out on the free plan.</span>
                 </span>
               </label>
...
function LiveServiceNowFetcher({
  onFetched,
}: {
  onFetched: (filename: string, xml: string) => void;
}) {
  const [module, setModule] = useState<"incident" | "problem" | "change" | "knowledge" | "service_catalog">("incident");
  ...
  async function handleFetch() {
    ...
    if (!val) {
      setStatus({ kind: "err", text: "Enter a record number first." });
      return;
    }
    setLoading(true);
    setStatus(null);
    try {
      const res = await fetch("/api/connection/fetch", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ module, number: val }),
      });
      const data = await res.json() as { ok: boolean; filename?: string; xml?: string; error?: string };
      if (!res.ok || !data.ok) {
        throw new Error(data.error || `Failed to fetch from ServiceNow (${res.status})`);
      }
      onFetched(data.filename!, data.xml!);
      setNumber("");
      setStatus({ kind: "ok", text: `Successfully synced ${data.filename} live!` });
    } catch (e) {
      setStatus({
        kind: "err",
        text: e instanceof Error ? e.message : "ServiceNow REST request failed.",
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="mt-4 rounded-2xl border border-line bg-slate-50 p-4">
      <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.16em] text-pine/60">
        Live ServiceNow REST Sync
      </p>
      <div className="mt-2.5 flex flex-wrap gap-2 sm:flex-nowrap">
        <select
          value={module}
          onChange={(e) => setModule(e.target.value as any)}
          className="rounded-xl border border-line bg-white px-2.5 py-2 text-xs font-semibold text-ink outline-none"
        >
          <option value="incident">Incident (INC)</option>
          <option value="problem">Problem (PRB)</option>
          <option value="change">Change (CHG)</option>
          <option value="knowledge">Knowledge (KB)</option>
          <option value="service_catalog">Service Catalog (RITM)</option>
        </select>
        <input
          value={number}
          onChange={(e) => setNumber(e.target.value)}
          placeholder={module === "incident" ? "INC0010001" : module === "problem" ? "PRB0001234" : "CHG0001234"}
          className="min-w-0 flex-1 rounded-xl border border-line bg-white px-3 py-2 text-xs font-mono text-ink outline-none"
        />
        <button
          type="button"
          onClick={handleFetch}
          disabled={loading}
          className="shrink-0 rounded-xl bg-pine px-4 py-2 font-mono text-xs font-bold text-paper shadow-sm hover:bg-pine-soft disabled:opacity-50"
        >
          {loading ? "Syncing..." : "Sync Live Record"}
        </button>
      </div>
      {status && (
        <p className={`mt-2 font-mono text-[10.5px] leading-4 ${status.kind === "ok" ? "text-emerald-700" : "text-rose-600"}`}>
