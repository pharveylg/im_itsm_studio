...
type UploadRequest = InitRequest | ChunkRequest | CompleteRequest | CancelRequest;
...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === "init") {
      ...
      if (!Number.isInteger(body.totalChunks) || body.totalChunks < 1 || body.totalChunks > MAX_CHUNKS) {
    ...
    if (body.action === "complete") {
      ...
      for (let index = 0; index < rows.length; index += 1) {
        if (rows[index].chunk_index !== index || rows[index].total_chunks !== body.totalChunks) {
          return NextResponse.json({ ok: false, error: "Upload chunks are incomplete or out of order." }, { status: 409 });
        }
      }
      ...
      if (fileBuffer.byteLength > MAX_FILE_BYTES) {
        return NextResponse.json({ ok: false, error: "Assembled file exceeds the guideline limit." }, { status: 413 });
      }
    ...
    const isDatabaseError = /database|postgres|relation|connect|timeout|ssl|password/i.test(message);
    return NextResponse.json(
      {
        ok: false,
        error: isDatabaseError
          ? `Storage error: ${message}. Verify the Vercel DATABASE_URL and Supabase schema.`
          : message,
      },
      { status: 500 },
    );
