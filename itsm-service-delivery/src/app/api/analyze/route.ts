...
type AnalyzeRequest = {
  ...
  focus?: string;
  includeRaw?: boolean;
  turbo?: boolean; // Vercel Hobby: skip raw excerpts, truncate guidelines, reduce output
...
export async function POST(request: Request) {
  try {
    ...
    // Serverless-safe output budget:
    // 3,200 tokens generates ~2,400 words of rich tagged-block analysis across all 10-12 tabs
    // and finishes in 12-22 seconds on Claude 3.5/Sonnet 4.6 and GPT-4o, well below Vercel's 60s limit.
    const turbo = Boolean(body.turbo);
    const maxTokens = turbo ? 2400 : Math.min(Math.max(providerConfig.maxTokens ?? 3200, 2000), 3600);
    const turboNotes = turbo
      ? "\nTURBO MODE: Provide a concise analysis. Limit each section to 3-5 key findings. Skip verbose commentary."
      : "";
    const turboGuidelines = guidelineBundle.text.slice(0, turbo ? 8_000 : 16_000);
    const turboContext = turbo || !body.includeRaw
      ? context.structuredContext
      : context.rawContext.slice(0, 20_000);

    // Warn about large requests upfront.
    const contextChars = turboContext.length;
    const estimatedOutputTokens = maxTokens;
    const estimatedTotalTimeSec = Math.round(0.3 + (contextChars / 3000) + (estimatedOutputTokens / 800));
    if (estimatedTotalTimeSec > 50) {
      console.warn(`[analyze] Estimated ${estimatedTotalTimeSec}s; consider turbo mode.`);
    }
    ...
    const analysisRequest = analysisMode === "mi_comms" && communications
      ? {
          systemPrompt: miCommsSystemPrompt() + turboNotes,
          userPrompt: miCommsUserPrompt({
            guidelines: turboGuidelines,
            focus: body.focus,
            scopedModules,
            itsmContext: turboContext,
            communications,
          }),
      ...
      : {
          systemPrompt: analysisSystemPrompt(scopedModules) + turboNotes,
